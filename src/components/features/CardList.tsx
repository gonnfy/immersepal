...
import { useGenerateTts, type TtsPayload, type TtsSuccessResponse } from '@/hooks/useGenerateTts';
import { useSaveAiContent, type SaveAiContentVariables } from '@/hooks/useSaveAiContent';
...
export function CardList({ deckId }: CardListProps) {
  ...
  const deleteCardMutation = useDeleteCard(deckId, {
    onSuccess: (deletedCardId: string) => {
      ...
      setIsDeleteDialogOpen(false); setCardToDelete(null);
  ...
  const { mutate: saveAiContentMutate, isPending: isSavingContent } = useSaveAiContent({
    onSuccess: (_data: AICardContentApiResponse, variables: SaveAiContentVariables) => {
    ...
    onError: (error: AppError, variables: SaveAiContentVariables) => {
      console.error(`[CardList] Failed to save AI content for card ${variables.cardId}:`, error);
  ...
  const { mutate: generateTtsMutate, isPending: ttsIsPending } = useGenerateTts({
    ...
    onSuccess: (data: TtsSuccessResponse, variables: TtsPayload, _context: unknown) => { // ★ _context を追加 ★
      ...
      saveAiContentMutate({
        ...
        contentType: variables.side === 'front' ? AiContentType.AUDIO_PRIMARY : AiContentType.AUDIO_SECONDARY,
        language: variables.language || (variables.side === 'front' ? 'en-US' : 'ja-JP'),
        content: data.gcsPath
      });
      // ローディング解除は saveAiContentMutate で行う
    },
    onError: (error: AppError, variables: TtsPayload, _context: unknown) => { // ★ _context を追加 ★
      console.error(`[CardList] TTS generation failed for card ${variables.cardId}:`, error);
      setTtsErrorMsg(`Audio generation failed: ${error.message}`);
      setLoadingAudioId(null); // ローディング解除
    },
  });

  const { data: signedUrlData, isLoading: isFetchingUrl, error: fetchUrlError } = useGetTtsUrl(gcsPathToFetch);

  // Effects
   useEffect(() => {
    if (signedUrlData?.signedUrl) { setUrlToPlay(signedUrlData.signedUrl); setGcsPathToFetch(null); }
   }, [signedUrlData]);
   useEffect(() => {
    if (fetchUrlError) { console.error('[CardList] Failed to get signed URL via hook:', fetchUrlError); setTtsErrorMsg(`Failed to get audio URL: ${fetchUrlError.message}`); setLoadingAudioId(null); setGcsPathToFetch(null); }
   }, [fetchUrlError]);
  useEffect(() => {
    if (urlToPlay) {
      ...
      try {
        ...
        audio.onerror = () => { console.error("Audio element error"); setTtsErrorMsg("Audio playback failed."); setLoadingAudioId(null); }
      } catch (audioError) { console.error("Error initiating audio playback:", audioError); setTtsErrorMsg("Audio playback failed."); setLoadingAudioId(null); }
       finally { setUrlToPlay(null); }
    }
  }, [urlToPlay]);
  ...
  const handleEditClick = (card: Card) => {
    ...
    setEditingCard(cardForModal); setIsEditModalOpen(true);
  };
  const handlePlayAudio = (card: Card, side: 'front' | 'back') => {
    const languageCode = side === 'front' ? (process.env.NEXT_PUBLIC_TTS_LANGUAGE_CODE_EN || 'en-US') : (process.env.NEXT_PUBLIC_TTS_LANGUAGE_CODE_JA || 'ja-JP');
    const contentTypeToFind = side === 'front' ? AiContentType.AUDIO_PRIMARY : AiContentType.AUDIO_SECONDARY;
    const loadingId = `${card.id}-${side}`;
    if (loadingAudioId) return;
    setTtsErrorMsg(null);
    const existingAudio = card.aiContents?.find(c => c.contentType === contentTypeToFind && c.language === languageCode);
    const gcsPath = existingAudio?.content;
    if (gcsPath) {
      setLoadingAudioId(loadingId); setGcsPathToFetch(gcsPath);
    } else {
      const text = side === 'front' ? card.front : card.back;
      if (text && text.trim().length > 0) {
        setLoadingAudioId(loadingId);
        generateTtsMutate({ text, language: languageCode, cardId: card.id, side });
      } else { setTtsErrorMsg("No text available to synthesize."); }
    }
  };
  ...
  if (!hasCards) { return <p className="text-gray-500 dark:text-gray-400 text-center py-4">No cards found in this deck yet.</p>; }
    ...
    <div>
      ...
      <ul className="space-y-4">
        {cards.map((card: Card) => {
          ...
          const isDeleting = deleteCardMutation.isPending && cardToDelete === card.id;
          // Calculate disable conditions directly
          const isAnyAudioProcessing = isFetchingUrl || ttsIsPending || isSavingContent;
            ...
            <li key={card.id} className="p-4 border rounded-lg shadow-sm bg-white dark:bg-gray-800 dark:border-gray-700 transition hover:shadow-md">
              <div className="flex justify-between items-start gap-4">
                <div className="flex-grow">
                  ...
                  <div className="mb-2">
                    <div className="flex items-center gap-2">
                      ...
                      <button type="button" onClick={() => handlePlayAudio(card, 'front')} disabled={disablePlayButton || !card.front} className="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 disabled:opacity-50 disabled:cursor-not-allowed p-0.5" aria-label="Play front audio" title="Play front audio">
                        {isLoadingFront ? ( <ArrowPathIcon className="h-4 w-4 animate-spin" /> ) : ( <SpeakerWaveIcon className="h-4 w-4" /> )}
                      </button>
                    </div>
                  ...
                  <div className="mb-3">
                     <div className="flex items-center gap-2">
                        <p className="font-semibold text-sm text-gray-500 dark:text-gray-400">Back</p>
                         <button type="button" onClick={() => handlePlayAudio(card, 'back')} disabled={disablePlayButton || !card.back} className="text-gray-400 hover:text-gray-600 dark:text-gray-500 dark:hover:text-gray-300 disabled:opacity-50 disabled:cursor-not-allowed p-0.5" aria-label="Play back audio" title="Play back audio">
                          {isLoadingBack ? ( <ArrowPathIcon className="h-4 w-4 animate-spin" /> ) : ( <SpeakerWaveIcon className="h-4 w-4" /> )}
                        </button>
                      </div>
                         ...
                         <div className="flex flex-wrap gap-1.5">
                           {card.aiContents.map((content) => (
                             <span key={content.id} className="inline-flex items-center px-2 py-0.5 rounded-md text-xs font-medium bg-gray-100 text-gray-700 dark:bg-gray-700 dark:text-gray-200" title={`Type: ${content.contentType}, Lang: ${content.language}`}>
                               {content.contentType === AiContentType.EXPLANATION ? 'Expl.' : content.contentType === AiContentType.TRANSLATION ? 'Transl.' : content.contentType.replace('AUDIO_','Aud.')} ({content.language})
                             </span>
                           ))}
                         </div>
                       </div>
                ...
                <div className="flex flex-col space-y-2 flex-shrink-0">
                   <button type="button" onClick={() => handleEditClick(card)} disabled={disableEditDeleteButton} className="px-2.5 py-1 text-xs font-medium text-center text-yellow-700 bg-yellow-100 rounded hover:bg-yellow-200 focus:ring-4 focus:outline-none focus:ring-yellow-300 dark:bg-yellow-900/50 dark:text-yellow-300 dark:hover:bg-yellow-800/50 dark:focus:ring-yellow-800 disabled:opacity-50 disabled:cursor-not-allowed"> Edit </button>
                   <button type="button" onClick={() => handleDeleteClick(card.id)} disabled={disableEditDeleteButton} className="px-2.5 py-1 text-xs font-medium text-center text-red-700 bg-red-100 rounded hover:bg-red-200 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-900/50 dark:text-red-300 dark:hover:bg-red-800/50 dark:focus:ring-red-800 disabled:opacity-50 disabled:cursor-not-allowed"> {isDeleting ? '...' : 'Delete'} </button>
                </div>
              </div>
               {/* SRS Info */}
