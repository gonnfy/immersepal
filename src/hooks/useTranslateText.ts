...
const translateTextApi = async (
  ...
  console.log('[useTranslateText] Calling API:', apiUrl, 'with payload:', payload);
  ...
  if (!response.ok) {
    const errorData: { message?: string, errorCode?: string, details?: unknown } = { message: 'Failed to translate text', errorCode: 'API_ERROR', details: null };
    try {
      ...
      console.warn("[useTranslateText] Could not parse error response body:", e);
    }
  ...
  if (!result.success || typeof result.translation !== 'string') {
    console.error('[useTranslateText] Invalid success response format:', result);
    throw new AppError('Received invalid response format from translation API.', 500, ERROR_CODES.INTERNAL_SERVER_ERROR);
  }
  return result;
};
  ...
  >({
    ...
    onError: (error) => {
      ...
      if (error instanceof AppError) {
          ...
          console.error('An unexpected non-AppError occurred during translation:', error);
          // Optionally wrap it in AppError before calling onError
          const unknownError = new AppError('An unexpected error occurred during translation.', 500, 'INTERNAL_SERVER_ERROR', error);
