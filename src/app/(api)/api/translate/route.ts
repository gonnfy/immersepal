...
const translateApiSchema = z.object({
  ...
  sourceLanguage: z.string().min(2, { message: 'Source language is required.' }),
  targetLanguage: z.string().min(2, { message: 'Target language is required.' }),
});
  ...
  try {
    ...
    try {
      ...
      if (!validation.success) {
        throw new ValidationError('Invalid request body for translation.', validation.error.flatten());
      }
      payload = validation.data;
    } catch (e) {
      if (e instanceof ValidationError) { throw e; }
      console.error('Error parsing/validating translation request body:', e);
      throw new ValidationError('Invalid JSON body or structure for translation.');
    }

    // --- ↓↓↓ サービス呼び出しと Result 処理を修正 ↓↓↓ ---

    // 2. Call Translation Service (which now returns Result)
    const translationResult: Result<string, AppError> = await generateTranslation(
      payload.text,
    ...
    if (!translationResult.ok) {
      ...
      console.warn(`Translation API failed for text "${payload.text}":`, translationResult.error.message);
