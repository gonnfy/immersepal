...
const ttsApiSchema = z.object({
  ...
  language: z.string().min(2, { message: 'Language code is required (e.g., en-US, ja-JP).' }),
});
...
export async function POST(request: Request) {
  try {
    ...
    if (!ttsResult.ok) {
      ...
      console.error(`TTS Service failed for text "${payload.text}":`, ttsResult.error.message);
