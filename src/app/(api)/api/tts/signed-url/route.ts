...
try {
    ...
    if (!bucketName) {
        console.error("GCS_BUCKET_NAME missing in environment for signed URL generation.");
        storage = null;
    }
} catch(error){
    console.error("Failed to initialize Storage client for signed URL generation.", error);
    storage = null;
}
...
export async function GET(request: NextRequest) {
  if (!storage || !bucketName) {
      return handleApiError(new ExternalApiError("Storage client is not initialized or bucket name is missing."));
  }
  ...
  try {
    ...
    if (!gcsPath || typeof gcsPath !== 'string' || !gcsPath.startsWith('tts-audio/')) {
